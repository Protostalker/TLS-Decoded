  ...
  return (
    <div style={{
      ...
      {loading && rows.length === 0 && (
        <div style={{ textAlign: 'center', color: 'var(--brand-text-dimmer, #64748b)', padding: 24, fontSize: 12 }}>Loading…</div>
      )}
      {error && <div style={{ color: '#ef4444', fontSize: 12, padding: 12 }}>Error: {error}</div>}
      {!loading && !error && rows.length === 0 && (
        <div style={{ textAlign: 'center', color: 'var(--brand-text-dimmer, #64748b)', padding: 24, fontSize: 12 }}>No readings yet</div>
      )}
