...
3. **One-way sync only for v1** — station pushes to cloud, cloud never pushes commands/config back down, **with one narrow exception now built: pricing.** A price entered on the cloud's T1 dashboard queues as a `PendingPriceUpdate`; the station's `sync` container polls for it every tick and applies it locally (see `cloud/README.md`'s Pricing section). Broader remote settings changes (poll interval, tank capacity, etc.) from the cloud side remain a v2 problem — this was kept intentionally narrow rather than building a general remote-config channel. Branding (colors/logo) is a *station-owned config* field, not telemetry, but flows in the same one-way direction as everything else — station pushes it up as part of the normal batch (`station_info`), cloud only ever mirrors it, never sets it (see `cloud/README.md`'s Branding section).
