...
License status + activation surface for the Cloud Utility's own UI.
  ...
  - /api/license/config, /activate, /deactivate — admin only. Submit or
    clear the license passphrase from the UI — CLOUD_LICENSE_KEY only ever
    seeds this once, on a brand new deployment; every change after that
    goes through here. See licensing.py's module docstring.
"""
    ...
    if degraded:
        message = (
            ...
            + ("Only admins can currently view data — contact your admin, or renew to restore full access."
               ...
               "Activate a valid license to restore full functionality. See Admin -> License.")
        )
...
# ── Activation — submit a passphrase from the UI instead of editing env
#    vars. CLOUD_LICENSE_KEY only seeds the very first boot; everything
#    after that goes through here. See licensing.py's module docstring. ──
...
def activate_license(body: ActivateRequest, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    try:
        return licensing.activate(db, body.passphrase)
...
def deactivate_license(admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    switching to a different code or decommissioning, not part of the
    normal renewal flow."""
